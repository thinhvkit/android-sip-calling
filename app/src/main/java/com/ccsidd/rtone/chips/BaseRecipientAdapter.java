...
package com.ccsidd.rtone.chips;
...
import com.ccsidd.rtone.chips.DropdownChipLayouter.AdapterType;
...
public class BaseRecipientAdapter extends BaseAdapter implements Filterable, AccountSpecifier {
    ...
    protected static class TemporaryEntry {
        ...
        public TemporaryEntry(Contact contact, Long directoryId) {
            this.displayName = contact.getDisplayName();
            this.destination = contact.getDisplayName();
            this.destinationType = contact.getPhoneNumbers().first().getType();
            this.destinationLabel = contact.getPhoneNumbers().first().getLabel();
            this.contactId = contact.getId();
            this.directoryId = directoryId;
            this.dataId = -1;
            this.thumbnailUriString = contact.getImageUri();
            ...
            try {
                if (contacts != null) {
                    ...
                    for (Contact contact : contacts) {
                        ...
                        if (contact.getDisplayName().toLowerCase(current).contains(constraint.toString().toLowerCase(current))) {
                            for (Phone phone : contact.getPhoneNumbers()) {
                                putOneEntry(new TemporaryEntry(contact.getDisplayName(), phone.getNumber(),
                                                phone.getType(), phone.getLabel(), contact.getId(),
                                                null /* directoryId */, -1, contact.getImageUri(), 21, ""),
                                        true, entryMap, nonAggregatedEntries, existingDestinations);
                            }

                        } else {
                            for (Phone phone : contact.getPhoneNumbers()) {
                                if (phone.getNumber().replaceAll("[-\\s]", "").contains(constraint)) {
                                    putOneEntry(new TemporaryEntry(contact.getDisplayName(), phone.getNumber(),
                                                    phone.getType(), phone.getLabel(), contact.getId(),
                                                    null /* directoryId */, -1, contact.getImageUri(), 21, ""),
                                            true, entryMap, nonAggregatedEntries, existingDestinations);
    ...
    public BaseRecipientAdapter(ArrayList<Contact> contacts, Context context, int preferredMaxResultCount) {
        mContext = context;
    ...
    private List<RecipientEntry> constructEntryList(
        ...
        for (Map.Entry<Long, List<RecipientEntry>> mapEntry : entryMap.entrySet()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                entries.add(entry);
                tryFetchPhoto(entry, mContext, this, false, i);
                validEntryCount++;
            }
...
//                }
    ...
    public static void tryFetchPhoto(final RecipientEntry entry, Context mContext, BaseAdapter adapter, boolean forceLoad, int position) {
        if (forceLoad || position <= 20) {
            ...
            if (photoThumbnailUri != null) {
                ...
                if (photoBytes != null) {
                    ...
                    fetchPhotoAsync(entry, photoThumbnailUri, adapter, mContext);
                }
            }
        }
    }
    ...
    private static void fetchPhotoAsync(final RecipientEntry entry, final Uri photoThumbnailUri, final BaseAdapter adapter, final Context mContext) {
        final AsyncTask<Void, Void, byte[]> photoLoadTask = new AsyncTask<Void, Void, byte[]>() {
            @Override
            protected byte[] doInBackground(Void... params) {
                Bitmap bitmap = Utility.loadBitmapFromPath(mContext, photoThumbnailUri.getPath());
                if (bitmap != null) {
