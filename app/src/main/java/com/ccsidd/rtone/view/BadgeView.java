...
public class BadgeView extends TextView {
    ...
    public BadgeView(Context context, PagerSlidingTabStrip target,
                     int index) {
        this(context, null, android.R.attr.textViewStyle, target, index);
    }
    ...
    private void applyTo(View target) {
        ...
        if (target instanceof PagerSlidingTabStrip) {

            // set target to the relevant tab child container
            target = ((PagerSlidingTabStrip) target).getTabsContainer().getChildAt(targetTabIndex);
            this.target = target;

            ((ViewGroup) target).addView(this);
